...
class $Step4 extends Stepper {
  ...
  async run() {
    return new Promise(async (resolve,reject) => {
      ...
      if (!elementText) {
        LoggerUtils.sendLog("[Step #4] Google giriş sayfası açılamadı!", ConsoleLogStatus.DANGER);
        reject(false);
      } else {
        if (elementText.includes(data.LOGIN_SELECTORS.LOGIN)) {
          ...
          LoggerUtils.sendLog("[Step #4] Giriş seçeneği bulunamadı!", ConsoleLogStatus.DANGER);
          reject(false);
        }
      }
    })
