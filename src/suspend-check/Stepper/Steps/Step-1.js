...
class $Step1 extends Stepper {
  ...
  async run() {
    return new Promise(async (resolve,reject) => {
      ...
      if (!elementText) {
        LoggerUtils.sendLog("[Step #1] Google giriş sayfası açılamadı!", ConsoleLogStatus.DANGER);
        reject(false);
      } else {
        resolve(true);
        ...
        if (elementText.includes(data.LOGIN_SELECTORS.LOGIN)) {
          ...
          LoggerUtils.sendLog("[Step #1] Giriş seçeneği bulunamadı!", ConsoleLogStatus.DANGER);
          reject(false);
        }
      }
    })
